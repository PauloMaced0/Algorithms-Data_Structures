...
fraction best_rational_approximation_slow(double x,uint max_den, uint *numOftests)
{
...
fraction best_rational_approximation_fast(double x,uint max_den,uint *numOftests)
{
...
int main(void)
{
  ...
  while(1)
  {
    ...
    f = best_rational_approximation_slow(x,max_den,numOftests);
    printf("    slow: %u/%u (%.6e)\n",f.num,f.den,(double)f.num / (double)f.den - x);
    printf("\tSlow version made %d tests\n", numOftests[0]);

    f = best_rational_approximation_fast(x,max_den,numOftests);
    printf("    fast: %u/%u (%.6e)\n",f.num,f.den,(double)f.num / (double)f.den - x);
    printf("\tFast version made %d tests\n", numOftests[1]);
