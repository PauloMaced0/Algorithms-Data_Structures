...
int main(void)
{
  ...
  const int spaceAllocForIntArray = 250000;
  int *sieve = (int *)malloc((size_t)limit); //Espaço para 250000 inteiros  
  if(sieve == NULL)
    return 1;
  for(int i = 0;i < spaceAllocForIntArray;i++)
    sieve[i] = 0;
  sieve[0] = sieve[1] = 1; // 0 and 1 are not prime
  for(int p = 2;p * p <= spaceAllocForIntArray;p++)
    if(sieve[p] == 0)
      for(int i = p * p;i <= spaceAllocForIntArray;i += p)
